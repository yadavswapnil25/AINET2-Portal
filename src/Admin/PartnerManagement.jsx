...
const PartnerManagement = () => {
  ...
  const savePartner = async () => {
    try {
      ...
      let res;
      if (editing) {
        res = await partnerAPI.updatePartner(editing.id, {
          name: form.name,
          logoFile,
        ...
        res = await partnerAPI.createPartner({
          name: form.name,
          logoFile,
          subtitle: form.subtitle,
          link_url: form.link_url,
  ...
  const requestDelete = (ids) => {
    ...
    setConfirmState({ open: true, ids: idsArr, message: `Delete ${count} partner${count>1?'s':''}?`, confirming: false });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      let res;
      if (ids.length === 1) {
        res = await partnerAPI.deletePartner(ids[0]);
      } else {
        res = await partnerAPI.bulkDeletePartners(ids);
      }
      if (res.status) {
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3">
                  <input type="checkbox" checked={partners.length>0 && selectedIds.length===partners.length} onChange={toggleSelectAll} />
                </th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {partners.map(p => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 whitespace-nowrap">
                    {p.logo_url ? (
                      <img src={p.logo_url} alt={p.name} className="h-16 w-16 object-contain rounded" />
                    ) : (
                      <div className="h-16 w-16 bg-gray-100 rounded flex items-center justify-center text-gray-400"><ImageIcon size={18} /></div>
                    )}
                  </td>
                  <td className="px-6 py-3 text-sm font-medium text-gray-900">{p.name}</td>
                  <td className="px-6 py-3 text-sm text-gray-700">{p.subtitle || '-'}</td>
                  <td className="px-6 py-3 text-sm text-teal-700 truncate max-w-xs">
                    {p.link_url ? (
                      <a href={p.link_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1">
                        <Link2 size={14} /> {p.link_url}
                      </a>
                    ) : '-'}
                  </td>
                  <td className="px-6 py-3 text-sm text-gray-700">{p.sort_order ?? 0}</td>
                  <td className="px-6 py-3 whitespace-nowrap">
                    {p.is_active ? (
                  ...
                  <td className="px-6 py-3 whitespace-nowrap text-sm font-medium flex items-center gap-2">
                    <button onClick={() => openEdit(p)} className="px-3 py-1.5 text-xs rounded bg-gray-100 hover:bg-gray-200">Edit</button>
                    <button onClick={() => requestDelete(p.id)} className="px-3 py-1.5 text-xs rounded bg-red-50 text-red-700 hover:bg-red-100">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
            <div className="p-4 border-b">
              <h3 className="text-lg font-semibold">{editing ? 'Edit Partner' : 'New Partner'}</h3>
            </div>
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">Name *</label>
                <input type="text" value={form.name} onChange={(e)=>setForm(f=>({ ...f, name: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Subtitle</label>
                <input type="text" value={form.subtitle} onChange={(e)=>setForm(f=>({ ...f, subtitle: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="e.g., Leading EdTech Organization" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Logo {editing && '(leave empty to keep existing)'}</label>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => {
            ...
            <div className="p-4 border-t flex justify-end gap-2">
              ...
              <button onClick={savePartner} className="px-4 py-2 rounded bg-teal-600 hover:bg-teal-700 text-white text-sm">Save</button>
            </div>
          </div>
        </div>
      )}
